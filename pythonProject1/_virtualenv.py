...
def abrir_tela_cadastro():
    ...
    cadastro_window = tk.Toplevel(root)  # Define a janela principal como o pai da nova janela
    ...
    # Adicionando o comentário acima do botão "Cadastrar"
...
def exibir_qrcode_apos_cadastro():
    ...
    # Exibir a imagem do QR Code em um Label
    qr_code_label = tk.Label(qr_code_window, image=qr_code_image)
    qr_code_label.image = qr_code_image  # Garantir que a imagem não seja liberada da memória
    qr_code_label.pack()

def abrir_tela_verificacao(login, senha):
    verificacao_window = tk.Toplevel(root)  # Define a janela principal como o pai da nova janela
...
# Ajustando a aparência dos widgets para simular o estilo do Windows XP
